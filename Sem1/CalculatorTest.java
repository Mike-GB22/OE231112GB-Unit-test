...
//        ).isInstanceOf(IllegalStateException.class);
        ...
        //Домашка!!! Проверяем что при отрицательной цене выкинет исключение
        assertThatThrownBy(() ->
                 seminars.first.Calculator.Calculator.calculatingDiscount(-10.0, 10)
        ).isInstanceOf(ArithmeticException.class);

        //Домашка!!! Проверяем что при отрицательной скидке выкинет исключение
        assertThatThrownBy(() ->
                 seminars.first.Calculator.Calculator.calculatingDiscount(10.0, -10)
        ).isInstanceOf(ArithmeticException.class);

        //Домашка!!! Проверяем что цена правильно расчитывается со скидкой
        assertThat(Calculator.calculatingDiscount(10.0, 10)).isEqualTo(9);
